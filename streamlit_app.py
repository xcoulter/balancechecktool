...
def fetch_erc20(rpc, token, addr, blk):
    ...
    try: return c.functions.balanceOf(Web3.to_checksum_address(addr)).call(block_identifier=blk)
    except: return None
...
for i, r in df.iterrows():
    ...
    try:
        if _is_native(token):
            ...
            raw = fetch_erc20(RPC_URL, token, addr, blk) or 0
            bal = raw / 1e18
            sym = r.get(cmap.token_symbol,"TOKEN")
        res.append({"wallet": addr,"token": sym,"reported": rep,"onchain": bal,"delta": (bal-rep if rep is not None else None)})
    except Exception as e:
        res.append({"wallet": addr,"token": token,"error": str(e)})
